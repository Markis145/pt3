...
class llibre extends connexio{
		...
		function inicialitza($id){
			...
			if ($this->lli_idllibre==0) {//si és 0 vol dir que es crea una nova instància i per tant cal inicialitzar els atributs	
				$this->lli_llibre="";
			}
			else {
				// sino és 0 vol dir que volem consultar un element que ja tenim en la bd
				$sql="SELECT LLIBRES.LLI_IDLLIBRE, LLIBRES.LLI_LLIBRE ".
